...
class Transaction:
    def __init__(self,seter, name, vister):
        self.set = seter
        self.name = name
        self.vister = vister
...
class BlockChain:
    ...
    def create_genesis_block(self):
        ...
        new_block = Block('Hello World!', self.difficulty, 'lkm543')
        new_block.hash = self.get_hash(new_block, 0)
        self.chain.append(new_block)

    def initialize_transaction(self, seter, name, vister):
        # No need to check balance
        new_transaction = Transaction(seter, name, vister)
        return new_transaction

    def transaction_to_string(self, transaction):
        transaction_dict = {
            'seter': str(transaction.seter),
            'name': str(transaction.name),
            'vister': str(transaction.vister),
        }
    ...
    def add_transaction_to_block(self, block):
        ...
        self.pending_transactions.sort(key=lambda x: x.vister, reverse=True)
    ...
    def mine_block(self, miner):
        ...
        new_block = Block(last_block.hash, self.difficulty, miner)
    ...
    def get_visters(self, name):
        visters = 0
        run = 0
        for block in self.chain:
            for transaction in block.transactions:
                if transaction.name == name:
              	    visters+=transaction.vister
                run+=1
        print(run)
        return visters
    ...
    def receive_socket_message(self, connection, address):
        with connection:
            ...
            while True:
                ...
                if message:
                    if parsed_message["request"] == "get_visters":
                        print("Start to get the visters for client...")
                        name = parsed_message["name"]
                        vister = self.get_visters(name)
                        response = {
                            "name": name,
                            "vister": vister
                        }
